...
class BPMF(object):
    ...
    def fit(self, train_vec, test_vec, train_order, test_order, num_user, num_item):
        ...
        if ((not incremental) or (self.w_Item is None)):
            ...
            self.alpha_Item = 0.1 * np.random.rand(num_item, 1)   # item teaching ability
            ...
            for batch in range(self.num_batches):  # 每次迭代要使用的数据量
                ...
                if  self.dynamic: 
                    for i in range(self.batch_size):
                        ...
                        if len(hist) > 10:
                            hist = hist[0:9]
                        temp = np.matmul(self.alpha_Item[hist], np.ones((1,self.num_feat)))
                ...
                x = np.sum(np.multiply(new_w_User, self.w_Item[batch_ItemID, :]), axis=1) + \
                    self.gamma_Item[batch_ItemID,:].flatten()  
                expnx = np.exp(-x)            
                ...
                Ix_gamma = 2 * np.reshape(gradlogloss,(self.batch_size,1)) + self._lambda * self.gamma_Item[batch_ItemID,:]
                       
                dw_Item = np.zeros((num_item, self.num_feat))
                dw_User = np.zeros((num_user, self.num_feat))
                dw_alpha = np.zeros((num_item, 1))
                dw_gamma = np.zeros((num_item, 1))

                # loop to aggreate the gradients of the same element
                for i in range(self.batch_size):
                    dw_Item[batch_ItemID[i], :] += Ix_Item[i, :]
                    dw_User[batch_UserID[i], :] += Ix_User[i, :]
                    dw_gamma[batch_ItemID[i], :] += Ix_gamma[i,:]
                    if self.dynamic:
                        hist = train_order[shuffled_order[i]]
                        if not hist:
                            continue
                        if len(hist) > 10:
                            hist = hist[0:9]
                        dw_alpha[hist,:] +=  gradlogloss[i] * np.matmul(self.w_Item[hist,: ], np.transpose(self.w_Item[[batch_ItemID[i]],:]))

                dw_alpha  += self._lambda * self.alpha_Item
                ...
                self.gamma_Item_inc = self.momentum * self.gamma_Item_inc + self.epsilon * dw_gamma / self.batch_size
                ...
                if batch == self.num_batches - 1:
                    ...
                    x = np.sum(np.multiply(new_w_User, self.w_Item[np.array(train_vec[:, 1], dtype='int32'), :]), axis=1) + \
                        self.gamma_Item[np.array(train_vec[:, 1], dtype='int32'),:].flatten()   
                    expnx = np.exp(-x)            
                    linkx = np.divide(1, (1+expnx))          
                    logx = np.log(linkx)
                    lognx = np.log(1-linkx)
                ...
                if batch == self.num_batches - 1:
                    ...
                    x = np.sum(np.multiply(new_w_User, self.w_Item[np.array(test_vec[:, 1], dtype='int32'), :]), axis=1) + \
                        self.gamma_Item[np.array(test_vec[:, 1], dtype='int32'),:].flatten()   
                    expnx = np.exp(-x)            
                    linkx = np.divide(1, (1+expnx))          
                    logx = np.log(linkx)
                    lognx = np.log(1-linkx)
